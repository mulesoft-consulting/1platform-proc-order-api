pipeline {
  ...
  environment {
    ...
    WORKER = "Small"
    APP_CLIENT_CREDS = credentials("$BRANCH_NAME-api-mgr-proc-order-api")
  }
  stages {
    stage('Prepare') {
      steps {
        configFileProvider([configFile(fileId: "${BRANCH_NAME}-proc-order-api.yaml", replaceTokens: true, targetLocation: './src/main/resources/config/configuration.yaml')]) {
